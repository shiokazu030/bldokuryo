...
DMMブックスをメイン導線にし、DLsiteは管理人おすすめなどの管理側データに作品IDがある場合だけ表示します。
...
通常ユーザー投稿ではDLsite作品IDを入力させません。管理人おすすめにDLsite作品IDを入れたい場合だけ、`adminPickCards` の各データに `dlsiteWorkId: "RJ..."` を追加します。
